...
\n`` ``` [language]\n[code]\n``` ``\n where ``[language]`` can be ``C``, ``Python``, etc. and ``[code]`` is... well, your code. 
\nClick on 
 to remove the message when you're done.")
	...
	async def on_raw_reaction_add(self, ctx):
		if ctx.member == self.client.user:
			return
		if ctx.emoji.name == '
' and self.sent == 1 and ctx.message_id == self.sent_image_message.id:
			await self.sent_text_message.delete()
			await self.sent_image_message.delete()
			self.sent = 0
async def setup(client):
	await client.add_cog(CodeFormat(client))
